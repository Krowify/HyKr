...
- [HarmlessValve/HV-dotfiles](https://github.com/HarmlessValve/HV-dotfiles) (no license file upstream; kept for personal desktop use) — source of the `blackturq` theme's palette, transcribed from their `hypr/colors.toml` and `waybar/style.css` into `Configs/configs/theme-switcher/themes/blackturq/colors.json`, and reference for that theme's flat full-width waybar layout and btop/cava colours. Their waybar stylesheet credits [basecamp/omarchy](https://github.com/basecamp/omarchy) for its own layout, so this repo's `templates/waybar/style.css.tpl` was rewritten against HyKr's placeholder set rather than copied. mako, walker, ghostty and their `kdeglobals`/`color-schemes` Qt theming were deliberately not carried over — see that theme's `README.md` for which HyKr component each would have collided with.

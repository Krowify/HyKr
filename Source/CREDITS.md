...
- [HarmlessValve/HV-dotfiles](https://github.com/HarmlessValve/HV-dotfiles) (no license file upstream; kept for personal desktop use) — source of the `blackturq` theme's palette, transcribed from their `hypr/colors.toml` and `waybar/style.css` into `Configs/configs/theme-switcher/themes/blackturq/colors.json`, and reference for that theme's btop and cava colours. Only the palette came across: the theme runs the shared Quickshell dock rather than a bar of its own, and mako, walker, ghostty and their `kdeglobals`/`color-schemes` Qt theming were deliberately not carried over — see that theme's `README.md` for which HyKr component each would have collided with.

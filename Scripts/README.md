...
- [`install.sh`](install.sh) — the single entrypoint: installs `yay` if missing, installs `pkg_core.lst` (and `pkg_extra.lst` if you opt in), runs `link_dots.sh`, then asks before running `extra/install_sddm_theme.sh` and `extra/setup_firewall.sh`.
